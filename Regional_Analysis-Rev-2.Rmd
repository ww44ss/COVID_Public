...
knitr::opts_chunk$set(echo = FALSE, fig.height = 3.2, fig.align = "center", message = FALSE, warning= FALSE, error = FALSE)
...
fips_list <- unique((plot_data%>%filter(state != "Puerto Rico"))$fips)
...
# )
  ...
  final_plot_data %>% write_csv(str_c(getwd(),"/","Fitted_Modeled_NYT_COVID.csv"))
```
...
daily_deaths_trend <- d_p %>%
  ...
  rowwise()%>%
  summarize(daily_deaths = mean(c(fit,fit,fit,fit, fit, fit, fit, fit,fit, actual))) %>%
  as.numeric() %>%
  round(0)%>%
  yo
...
daily_mortality_percent_trend = 100*daily_deaths_trend/daily_cases_trend
...
daily_deaths_per_capita <- ggplot(data = d_p, aes(x = date) ) + 
 ...
 geom_col(aes(y = 100000.*daily_deaths2/us_pop), color = "grey80", width = 0.7, alpha = 0.5)+
  ...
  labs(title = "Daily US Deaths per 100k Population", subtitle = str_c("Currently ", daily_deaths_per_100k_trend, " deaths per day per 100k"), y = "deaths per 100k per day")+ 
  annotate(geom="text", x=(ymd("2020-07-01")), 
             y=((100000*daily_deaths_trend/us_pop)+.1), 
...
daily_deaths <- ggplot(data = d_p, aes(x = date) ) + 
 geom_col(aes(y = daily_deaths2), color = "grey80", width = 0.7, alpha = 0.5)+
  ...
  labs(title = "Daily US Deaths ", subtitle = str_c(round(daily_deaths_per_100k_trend, 2), " deaths per 100k population per day"), y = "deaths per day")+ 
  annotate(geom="text", x=(ymd("2020-08-15")), 
             y=((daily_deaths_trend)), 
             label=str_c(daily_deaths_trend,  " deaths per day"), size = 3.0,
             color="steelblue4", hjust = 0, vjust = -1) +
  annotate(geom="text", x=(max(d_p$date) - days(10)), 
...
daily_cases_per_capita <- ggplot(data = d_p, aes(x = date) ) +
 geom_col(aes(y = 100000.*daily_cases2/us_pop), color = "grey80", width = 0.7, alpha = 0.5)+
  ...
  labs(title = "Daily US Cases per 100k Population", subtitle = str_c(daily_cases_trend, " cases per day"), y = "cases per 100k per day")+ 
  annotate(geom="text", x=(min(d_p$date) + days(5)), 
             y=((100000*daily_cases_trend/us_pop)), 
  ...
  annotate(geom="text", x=(max(d_p$date) - days(10)), 
             ...
             label=str_c("@WinstonOnData ",today()), size = 2.5,
             color="steelblue4", hjust = 1) +
  geom_hline(yintercept =(100000*daily_cases_trend/us_pop), color = "#A51541", size = 0.7, alpha = 0.4 ) +
  theme_bw()
...
daily_cases <- ggplot(data = d_p, aes(x = date) ) +
 geom_col(aes(y = daily_cases2), color = "grey80", width = 0.7, alpha = 0.5)+
  ...
  labs(title = "Daily US Cases", subtitle = str_c((100000*daily_cases_trend/us_pop)%>%round(1), " cases per 100k population per day"), y = "cases per day")+ 
  annotate(geom="text", x=(min(d_p$date) + days(5)), 
             y=((daily_cases_trend)), 
             label=str_c(daily_cases_trend,  " cases per day"), size = 3.0,
             color="steelblue4", hjust = 0, vjust = 1) +
  annotate(geom="text", x=(max(d_p$date) - days(10)), 
             y=(0.045*max(d_p$daily_cases)), 
             label=str_c("@WinstonOnData ",today()), size = 2.5,
             color="steelblue4", hjust = 1) +
  geom_hline(yintercept =(daily_cases_trend), color = "#A51541", size = 0.7, alpha = 0.4 ) +
  theme_bw()
...
p_daily_mortality <- ggplot(d_p%>%filter(date > ymd("2020-03-15")), 
  ...
  geom_line(color = "#C46232", size = 1.5, alpha = 0.8) + 
  labs(title = "Daily Deaths/Cases", y = "daily mortality (%)")+
  annotate(geom="text", x=(ymd("2020-5-01")), 
             y=daily_mortality_percent_trend, 
  ...
  annotate(geom="text", x=(max(d_p$date) - days(10)), 
             ...
             label=str_c("@WinstonOnData ",today()), size = 2.5,
             color="steelblue4", hjust = 1) +
  geom_hline(yintercept =daily_mortality_percent_trend, color = "#A51541", size = 0.7, alpha = 0.4 ) +
  ylim(0,9)+
  theme_bw()
...
p_total_deaths <- ggplot(d_p, 
  ...
  labs(title = "Total US Deaths", subtitle = str_c((round(100000*max(d_p$total_deaths)/us_pop, 1)), " deaths per 100k population "), y = "deaths")+
    annotate(geom="text", x=(max(d_p$date) - days(20)), 
...
p_r <- 
  ggplot(d_p%>%filter(date > ymd("2020-03-15")),
                       aes(x = date, y = r_e)) + geom_line(color = "#C46232", size = 0.8, alpha = 0.8) + geom_smooth(span = .15, size = 2, alpha = 0.8)+
  labs(title = "Effective Reproduction Rate", y = "R_e")+
  scale_y_log10()+
  annotate(geom="text", x=(ymd("2020-08-01")), 
             y=d_p[d_p$date == max(d_p$date),]$r_e[1], 
  ...
  annotate(geom="text", x=(max(d_p$date) - days(10)), 
             ...
             label=str_c("@WinstonOnData ",today()), size = 2.5,
             color="steelblue4", hjust = 1) +
  geom_hline(yintercept = d_p[d_p$date == max(d_p$date),]$r_e[1], color = "#A51541", size = 0.7, alpha = 0.4 ) +
  theme_bw()
...
```{r, fig.height=4, fig.width=9, fig.align="center", fig.height=3}
...
data_x <- final_plot_data %>%
  ...
  ungroup()%>%
  filter(date == max(date) | date == ymd("2020-08-15")| date == ymd("2020-11-07")) %>%
...
states <- states_cache
  ...
  plot_colors2 <- rev(plot_colors)
  plot_colors <-  brewer.pal(n = 9, name = "OrRd")
  plot_colors_deaths <- brewer.pal(n = 9, name = "OrRd")

data_x <- final_plot_data %>%
  group_by(ST) %>%
  filter(date == max(date)) %>%
  summarize(cases = sum(cases), daily_cases = sum(daily_cases), rolling_baseline_14 = sum(rolling_baseline_14), population = sum(population), deaths = sum(deaths), daily_deaths = sum(daily_deaths))%>%
  mutate(r_e = 12*daily_cases/rolling_baseline_14) %>%
  select(ST, cases, daily_cases, rolling_baseline_14, r_e, population, deaths, daily_deaths) %>%
  ...
  mutate(daily_deaths_per_capita = 10^5*daily_deaths/population) %>%
  mutate(short_term_change = 10^5*(1+(1-r_e)/2)*daily_cases/population)%>%
  yo
...
t_cases <-    
  ggplot() + 
    ...
    scale_fill_gradientn(colors =plot_colors, name = "Cases", limits = c(0, 2000000), na.value = "blue") + 
    theme_bw()+
...
t_cases_per_capita <-    
  ggplot() + 
    geom_sf(data = data_x, aes(geometry = geom, fill = 100000*cases/population), color = "white", size = 0.7, alpha = 0.85) + 
    coord_sf(xlim = c(as.numeric(bbox[1]), as.numeric(bbox[3])), ylim = c(as.numeric(bbox[2]), as.numeric(bbox[4])), expand = FALSE) +
    scale_fill_gradientn(colors =plot_colors, name = TeX("Cases/100k"), limits = c(0, 12000), na.value = "blue") + 
    theme_bw()+
    theme(axis.title.x=element_blank(),
          axis.text.x=element_blank(),
          axis.ticks.x=element_blank(),
...
t_deaths <-    
  ggplot() + 
    geom_sf(data = data_x, aes(geometry = geom, fill = deaths), color = "white", size = 0.6, alpha = 0.85) + 
    coord_sf(xlim = c(as.numeric(bbox[1]), as.numeric(bbox[3])), ylim = c(as.numeric(bbox[2]), as.numeric(bbox[4])), expand = FALSE) +
    scale_fill_gradientn(colors =plot_colors_deaths, name = TeX("Deaths"), limits = c(0, 40000), na.value = "blue") + 
    theme_bw()+
    theme(axis.title.x=element_blank(),
          axis.text.x=element_blank(),
          axis.ticks.x=element_blank(),
          ...
          strip.text = element_text(color = "#154555", size = 14, face = "bold")) +
    #labs(title = str_c("Reproduction Rate," TeX("$R_e$")),
      labs(title =  TeX("How Many People Have Died?"),
         subtitle = TeX("Total Deaths by State")) +
     annotate(geom="text", x=-72, 
             y=25, 
             label=str_c("@WinstonOnData - ", today() ), size = 2.5,
             color="Grey30") 
...
t_deaths_per_capita <-    
  ggplot() + 
    geom_sf(data = data_x, aes(geometry = geom, fill = 100000*deaths/population), color = "white", size = 0.6, alpha = 0.85) + 
    coord_sf(xlim = c(as.numeric(bbox[1]), as.numeric(bbox[3])), ylim = c(as.numeric(bbox[2]), as.numeric(bbox[4])), expand = FALSE) +
    scale_fill_gradientn(colors =plot_colors_deaths, name = TeX("Deaths/100k"), limits = c(0, 200), na.value = "blue") + 
    theme_bw()+
    theme(axis.title.x=element_blank(),
          axis.text.x=element_blank(),
          axis.ticks.x=element_blank(),
          ...
          strip.text = element_text(color = "#154555", size = 14, face = "bold")) +
    #labs(title = str_c("Reproduction Rate," TeX("$R_e$")),
      labs(title =  TeX("What Fraction People Have Died?"),
         subtitle = TeX("Total Deaths by State Per 100k Population")) +
     annotate(geom="text", x=-72, 
             y=25, 
             label=str_c("@WinstonOnData - ", today() ), size = 2.5,
             color="Grey30")  
...
p_reproduction <-    
  ggplot() + 
    ...
    coord_sf(xlim = c(as.numeric(bbox[1]), as.numeric(bbox[3])), ylim = c(as.numeric(bbox[2]), as.numeric(bbox[4])), expand = FALSE) +
    scale_fill_gradientn(colors =plot_colors2, name = TeX("log_2(R_e)"), limits = c(-0.75, 0.75), na.value = "blue") + 
    theme_bw()+
    theme(axis.title.x=element_blank(),
          axis.text.x=element_blank(),
          axis.ticks.x=element_blank(),
...
p_per_capita <-    
  ggplot() + 
    ...
    coord_sf(xlim = c(as.numeric(bbox[1]), as.numeric(bbox[3])), ylim = c(as.numeric(bbox[2]), as.numeric(bbox[4])), expand = FALSE) +
    scale_fill_gradientn(colors =plot_colors, name = TeX("Cases/100k"), limits = c(0, 150), na.value = "red") + 
    theme_bw()+
    theme(axis.title.x=element_blank(),
          axis.text.x=element_blank(),
          axis.ticks.x=element_blank(),
...
#     coord_sf(xlim = c(as.numeric(bbox[1]), as.numeric(bbox[3])), ylim = c(as.numeric(bbox[2]), as.numeric(bbox[4])), expand = FALSE) +
#     scale_fill_gradientn(colors =plot_colors, name = TeX("Cases/100k"), limits = c(0,250), na.value = "red") + 
#     theme_bw()+
#     theme(axis.title.x=element_blank(),
#           axis.text.x=element_blank(),
#           axis.ticks.x=element_blank(),
...
```{r, warning = F, message = F, error = F, fig.align='center', fig.height =14, fig.width = 8, echo = F, eval = T}

grid.arrange(t_cases, t_cases_per_capita, t_deaths, t_deaths_per_capita, nrow = 4)
...
```{r, warning = F, message = F, error = F, fig.align='center', fig.height =4.5, fig.width = 7, echo = F, eval = T }
...
## per capita cases
 ...
 state_abb <- bind_cols(state = state.name, ST = state.abb)
  ...
  plot_colors <- c("grey96","rosybrown1", "darkred")
  ...
  ggplot(crs = st_crs(102003)) + 
    ...
    labs(title = str_c('Total COVID-19 Cases per 100k Population'),
...
## per capita deaths
 ...
 state_abb <- bind_cols(state = state.name, ST = state.abb)
  ...
  #plot_colors <- rev(c(brewer.pal(n = 5, name = "RdYlGn"), "#3392B2"))
  plot_colors <- c("grey99","chocolate1", "darkred")
...
state_list <- data_x %>% 
  ungroup()%>%
  mutate(r_e = round(r_e,2), cases = round(cases,0 ), daily_cases = round(daily_cases, 0), daily_cases_per_100k = round(100000*daily_cases/population, 1))%>%
  arrange(desc(daily_cases_per_100k))%>%
  select (state, 'R_e'=r_e , cases, 'daily cases'= daily_cases, 'daily cases per 100k'= daily_cases_per_100k )%>%
  ungroup()%>%
  yo



kable(state_list, align = c("l", rep("c", 4))) %>% 
  column_spec(1, width = "1.5in") %>%
  column_spec(2, width = "0.5in") %>%
  column_spec(3:4, width = "1.0in") %>%
  column_spec(5, width = "1.5in") %>%
  kable_styling(full_width = F,  font_size = 12, bootstrap_options = c("hover", "striped")) %>% 
  row_spec(0, bold = T, color = "linen", background = "#FCAC6E") %>%
...
for (state_x in state_list) {
 ...
 pp<- ggplot(data = d_p,
         aes(x = fct_reorder(county, desc(cases)) , y = 100000*daily_cases/population, fill = as.factor(date)) )+
    geom_col(position = "dodge") + 
    labs(title = str_c("Cases per day per 100k population for top counties in ", state_x), x = "county", y = "cases per day per 100k", fill = "date") + scale_fill_manual(values = c("lightskyblue1", "lightskyblue4", "#DD442299"))+
   #scale_y_log10()+
...
p_daily_cases_per_capita_recent <-  ggplot(data = d_p %>% filter(date >= ymd("2020-09-01")), aes(x = date, y = 100000*total_daily/population + 0.1, color = state))  + 
  ...
  scale_y_log10(breaks = c(3, 10, 30, 100, 300), limits = c(3, 300))+
  geom_hline(yintercept = daily_cases_per_100k_trend, size = 0.5, color = "salmon", alpha = 0.5) +
  # annotate(geom="text", x=(ymd("2020-08-01")), 
  #            y=daily_cases_per_100k_trend*1.05, 
  #            label="National Average", size = 2.5,
...
p_daily_deaths_per_capita_recent <- ggplot(d_p %>% filter(date >= ymd("2020-09-01")), 
  ...
  labs(title = "Daily Deaths per 100k", y = "daily deaths")+
  scale_color_brewer(palette="Dark2")+
  scale_y_log10(breaks = c(0.03, 0.1, 0.3, 1, 3, 10), limits = c(0.03, 10)) +
  #ylim(0, 2.0) +
  scale_fill_brewer(palette="Dark2")+
  theme_bw()+ theme(legend.position="none")+
  geom_hline(yintercept = daily_deaths_per_100k_trend, size = 0.5, color = "salmon", alpha = 0.5) +
...
grid.arrange(p_total_cases, p_daily_cases_per_capita, p_total_deaths, p_daily_deaths_per_capita, p_daily_cases_per_capita_recent, p_daily_deaths_per_capita_recent, nrow = 3)
...
`r as.numeric (today()-ymd("2020-12-05"))` days ago: decreased max value of Daily Cases per 100k State map.  
`r as.numeric (today()-ymd("2020-12-12"))` days ago: increased max total state cases to 2,000,000 as California passes 1.5Million diagnosed cases.  

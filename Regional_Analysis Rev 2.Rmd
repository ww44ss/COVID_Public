...
state_list <- data_x %>% 
  ungroup()%>%
  mutate(r_e = round(r_e,2), cases = round(cases,0 ), daily_cases = round(daily_cases, 0), daily_cases_per_100k = 100000*daily_cases/population)%>%
  select (state, 'R_e'=r_e , cases, daily_cases, daily_cases_per_100k )%>%
...
```{r, warning=F, error=F, fig.align='center', message = F, error=F, warning = F, fig.height=3.5, fig.width = 7, eval = F}

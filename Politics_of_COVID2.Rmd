...
house_data_kaggle <- "1976_2018_house.csv"

house_results <- read_csv(str_c(getwd(),"/", house_data_kaggle), col_types = cols(
  year = col_double(),
  state = col_character(),
  ...
  filter(year == 2018) %>%
  select(state = state, ST = state_po, district = district, party, candidate_votes = candidatevotes, total_votes = totalvotes) %>%
  filter(!is.na(party)) %>%
  pivot_wider(names_from = party, values_from = candidate_votes) %>%
  yo
...
p_r <- 
  ggplot(d_p %>% filter(date >= ymd("2020-03-21")),
                       aes(x = date, y = r_e, color = party)) + geom_line(size = 0.3, alpha = 0.5) + geom_smooth(span = .25, se=FALSE, size = 2.5, alpha = 0.6)+
...
date_since <- "2020-07-04"

data_y <- final_plot_data %>%
  select(-lat, -lon)%>%
  group_by(state, ST, party) %>%
  #mutate(max_baseline = max(rolling_baseline_14)) %>%
  filter(date == ymd(date_since) ) %>%
  summarize(deaths = sum(deaths), daily_deaths = sum(daily_deaths), rolling_baseline_14 = sum(rolling_baseline_14), population = sum(population)) %>%
  ungroup() %>%
  select(ST, deaths) %>%
...
ggplot(state_list, aes(x = fct_reorder(ST, desc(per_capita)), y = per_capita, fill = party)) + 
  geom_col() + 
  scale_fill_manual(values = c("blue", "red")) + 
  labs(title = str_c("COVID Deaths / 100k Population since ", date_since, ", by State"), subtitle = "Governor's Party Affiliation Indicated by Color", x = "State", y = "Deaths per 100k") + 
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, size = 8, vjust = 0.5, hjust = 1), 
        legend.title = element_blank(),
        legend.text = element_text(color = "grey10", size = 10),
...
t_cases_per_capita <-    
  ggplot() + 
    geom_sf(data = data_x, aes(geometry = geom, fill = 100000* cases/population), color = "grey80", size = 0.6, alpha = 0.85) + 
    coord_sf(xlim = c(as.numeric(bbox[1]), as.numeric(bbox[3])), ylim = c(as.numeric(bbox[2]), as.numeric(bbox[4])), expand = FALSE) +
    scale_fill_gradientn(colors =plot_colors, name = TeX("Cases/100k"), limits = c(0, 3800)) + 
    theme_bw()+
...
p_per_capita <-    
  ggplot() + 
    geom_sf(data = data_x, aes(geometry = geom, fill = daily_cases_per_capita), color = "grey80", size = 0.6, alpha = 0.85) + 
    coord_sf(xlim = c(as.numeric(bbox[1]), as.numeric(bbox[3])), ylim = c(as.numeric(bbox[2]), as.numeric(bbox[4])), expand = FALSE) +
...
p_short_term_change <-      
  ggplot() + 
    geom_sf(data = data_x, aes(geometry = geom, fill = 100000*short_term_change/population), color = "grey80", size = 0.6, alpha = 0.85) + 
    coord_sf(xlim = c(as.numeric(bbox[1]), as.numeric(bbox[3])), ylim = c(as.numeric(bbox[2]), as.numeric(bbox[4])), expand = FALSE) +
    scale_fill_gradientn(colors =plot_colors, name = TeX("Cases/100k"), limits = c(0, 50), na.value = "red") + 
    theme_bw()+
    theme(axis.title.x=element_blank(),
...
While the State-Level Data tell as remarkable story, it is also interesting to look at County-level data. COunty level data on presidential elections are downloaded from [here](https://github.com/tonmcg/US_County_Level_Election_Results_08-16/blob/master/2016_US_County_Level_Presidential_Results.csv)

```{r}


President_2016<-read_csv(str_c(getwd(),"/", "President_2016.csv"))


President_2016 <- President_2016 %>%
  mutate(fips = as.character(combined_fips)) %>%
  mutate(fips = ifelse(nchar(fips) == 4, str_c("0", fips), fips )) %>%
  select(votes_gop, votes_dem, fips) %>%
  yo

final_plot_data <- final_plot_data %>%
  left_join(President_2016, by = "fips")
...
data_x <- final_plot_data %>%
  filter(date == max(date)) %>%
  mutate(party_sku = ((votes_gop-votes_dem)/(votes_gop + votes_dem))) %>%
  mutate(hilary_fraction = ((votes_dem)/(votes_gop + votes_dem))) %>%
  yo

since_date = "2020-09-01"

data_y <- final_plot_data %>%
  filter(date == ymd(since_date) ) %>%
  rename(baseline_deaths = deaths) %>%
  select(fips, baseline_deaths) %>%
  yo

data_x <- data_x %>%
  right_join(data_y, by = "fips") %>%
  mutate(preventable_deaths = deaths - baseline_deaths) %>%
  mutate(preventable_deaths = ifelse(is.na(preventable_deaths), 1, preventable_deaths)) %>%
  mutate(preventable_deaths = ifelse(preventable_deaths <1, 1, preventable_deaths)) %>%
  yo


ggplot(data = data_x, aes(x = 100*(1-hilary_fraction), y = 100000*(preventable_deaths+.1)/population, color = party_sku, size = population))+
scale_color_gradient(low = "blue", high = "red", name = "% Trump Vote") + 
  geom_point(alpha = 0.3) + 
  geom_smooth(method = "gam", formula = y ~ log10(x), linetype="dashed",
             color="darkred", fill="blue") + 
  labs(title = str_c("2016 Election Vote vs. COVID Deaths since ", since_date ), subtitle = str_c("Data Per US County. Size represents population. Data per ", data_x$date[1] ), y = "Deaths per 100k Population", x = "% Vote for Trump") + 
  theme_bw() + scale_y_log10() + 
  theme(legend.position = "none")+ 
  theme(legend.position = "none")+
     annotate(geom="text", x=78, 
             y=.025, 
             label=str_c("@WinstonOnData - ", today() ), size = 2.5,
             color="Grey30")
...
ggplot(data = data_x, aes(x = 100*(1-hilary_fraction), y = 100000*(daily_deaths+.1)/population, color = party_sku, size = population))+
scale_color_gradient(low = "blue", high = "red", name = "% Trump Vote") + 
  geom_point(alpha = 0.3) + scale_y_log10() + 
  geom_smooth(method = "gam", formula = y ~ log10(x), linetype="dashed",
             color="darkred", fill="blue") + 
  labs(title = str_c("2016 Election Vote vs. Daily COVID deaths on ", data_x$date[1]), subtitle = "Voter and COVID data per US County", y = str_c("daily deaths per 100k population"), x = "% Vote for Trump") + 
  theme_bw() + 
  theme(legend.position = "none")+
     annotate(geom="text", x=80, 
             y=.0075, 
             label=str_c("@WinstonOnData - ", today() ), size = 2.5,
             color="Grey30")
...
data_x <- final_plot_data %>% 
  ...
  mutate(cases = mean(cases), daily_cases = mean(daily_cases), deaths = mean(deaths), daily_deaths = mean(daily_deaths)) %>%
  filter(date == max(date)) %>%
  mutate(party_sku = ((votes_gop-votes_dem)/(votes_gop + votes_dem))) %>%
  mutate(hilary_fraction = ((votes_dem)/(votes_gop + votes_dem))) %>%
  mutate(winner = ifelse(hilary_fraction >=.5, "Clinton", "Trump")) %>%
  mutate(daily_deaths_per_100k = 100000*(daily_deaths)/population) %>%
  mutate(total_votes = votes_dem+votes_gop) %>%
  yo


ggplot(data = data_x, aes(x = votes_dem+votes_gop, y = 100000*(daily_deaths)/population, color = winner, size = population))+
scale_color_manual(values = c("blue", "red"), name = "Candidate") +  
  geom_point(alpha = 0.1) + scale_y_log10() + scale_x_log10() +
  geom_smooth(method = "gam", formula = y ~ x, linetype="dashed",
             color="darkred", fill="blue") + 
  labs(title = str_c("2016 Election Vote vs. Mean Daily COVID deaths on ", data_x$date[1]), subtitle = "Voter and COVID data per US County", y = str_c("daily deaths per 100k population"), x = "Total County Votes") + 
  theme_bw() + 
  theme(legend.position = "none")+
...
ggplot(data = data_x, aes(x = winner, y = 100000*(daily_deaths)/population))+
scale_fill_manual(values = c("blue", "red"), name = "Candidate") +  
  geom_boxplot(aes(fill = winner)) + scale_y_log10()+   
  labs(title = str_c("2016 Election Vote vs. Mean Daily COVID deaths on ", data_x$date[1]), subtitle = "Voter and COVID data per US County", y = str_c("daily deaths per 100k population"), x = "Total County Votes") + 
  theme_bw() 
...
ggplot(data = data_x, aes(x = 100000*(daily_deaths)/population))+
scale_fill_manual(values = c("blue", "red"), name = "Candidate") +  
  scale_x_log10()+
  geom_histogram(aes(y = ..density.., fill = winner), position = "dodge") +   
  labs(title = str_c("2016 Election Vote vs. Daily COVID deaths "), subtitle = "Voter and COVID data per US County", y = str_c("Density"), x = "Deaths per 100k Population") + 
  theme_bw() + 
  #theme(legend.position = "none")+
  geom_density(aes(fill = winner),alpha=.2) +
...
data_x <- final_plot_data %>%
  ...
  summarize(daily_cases = sum(daily_cases), cases = sum(cases), deaths = sum(deaths), daily_deaths = sum(daily_deaths), population = sum(population)) %>%

  yo


ggplot(data = data_x, aes(x = date, y = 100000*(daily_deaths+.1)/population, color = winner))+
scale_color_manual(values = c("blue", "red"), name = "Candidate") + 
  geom_line(alpha = 0.5, size = 2) +
  labs(title = str_c("2016 Election Vote vs. Daily COVID deaths on ", data_x$date[1]), subtitle = "Voter and COVID data per US County", y = str_c("daily deaths per 100k population"), x = "date") + 
  theme_bw() + 
  theme(legend.position = "none")+
     annotate(geom="text", x=ymd("2020-09-01"), 
             y=.0075, 
             label=str_c("@WinstonOnData - ", today() ), size = 2.5,
             color="Grey30")
...
county_election <- read_csv("https://raw.githubusercontent.com/tonmcg/US_County_Level_Election_Results_08-16/master/2016_US_County_Level_Presidential_Results.csv")
...
# House Of Representatives and Local Politics
...
`r as.numeric(today()-ymd("2020-09-21"))` days ago: Branched from Regional COVID ANALYSIS.
`r as.numeric(today()-ymd("2020-09-29"))` days ago: Added regression at county level.
`r as.numeric(today()-ymd("2020-10-11"))` days ago: Added density histograms of vote versus recent death rate per 100k.
`r as.numeric(today()-ymd("2020-10-12"))` days ago: Added link to this data on [recent House elections](https://www.kaggle.com/tunguz/us-elections-dataset?select=1976-2018-house2.csv)

...
## plot per capita cases as bubbles
...
## per capita cases
 ...
 state_abb <- bind_cols(state = state.name, ST = state.abb)
  ...
  plot_colors <- c("white","salmon", "darkred")
  ...
  counties <- counties_max %>%
    ...
    theme(axis.title.x=element_blank(),
          ...
          panel.grid.minor = element_blank(),
          strip.text = element_text(color = "#154555", size = 12, face = "bold")) +
    labs(title = str_c('PER CAPITA COVID-19 CASES: data through ', date_display),
         subtitle=str_c("NYTimes COVID19 Database"), "color = cases/100k") +
     annotate(geom="text", x=-70,
             y=25,
             label=str_c("ww44ss - ", today() ), size = 2.5,
 ...
 state_abb <- bind_cols(state = state.name, ST = state.abb)
  ...
  counties <- counties_max %>%
    ...
    theme(axis.title.x=element_blank(),
          ...
          panel.grid.minor = element_blank(),
          strip.text = element_text(color = "#154555", size = 12, face = "bold")) +
    labs(title = str_c('PER CAPITA COVID-19 MORTALITY: data through ', date_display),
         subtitle=str_c("NYTimes COVID19 Database"), "color = cases/100k") +
     annotate(geom="text", x=-70,
             y=25,
             label=str_c("ww44ss - ", today() ), size = 2.5,
...
states <- states %>% 
  ...
  range_lon = abs(bbox[1] - bbox[3])
    ...
    theme(axis.title.x=element_blank(),
          ...
          panel.grid.minor = element_blank(),
          strip.text = element_text(color = "#154555", size = 14, face = "bold")) +
    labs(title = str_c("Disease Baseline Control"),
...
p_total_cases <- ggplot(data = d_p, aes(x = date, y = total_cases) ) + 
  ...
  labs(title = "US Total Cases", subtitle = today(), y = "cases")+ 
    annotate(geom="text", x=(max(d_p$date) - days(50)), 
             ...
             label=str_c((max(d_p$total_cases)/1000000)%>% round(2), " million cases    "), size = 3.0,
             color="steelblue4") +
  theme_bw()
...
p_r <- ggplot(d_p,
                       aes(x = date, y = r_e)) + geom_line(color = "#5442E2", size = 1.5, alpha = 0.8) + 
...
data_x <- final_plot_data %>%
  ...
  mutate(control = 100*(1 - rolling_baseline_14/max(rolling_baseline_14))) %>%
  ungroup()%>%
  filter(date == max(date) | date == ymd("2020-07-15")| date == ymd("2020-05-18")) %>%
  filter(is.finite(r_e)) %>%
  mutate(date = as.factor(date))%>%
  filter(r_e < 3 & r_e>0)%>%
  select(r_e, date, control) %>%
...
# ggplot(data_x, aes(x = control, fill = date)) + geom_bar(stat = "bin", position = "dodge", bins =10, color = "transparent") + #geom_density(alpha = 0.2, bw = .04) + 
#   scale_fill_manual(values =  c("palegreen","steelblue2", "salmon")) + labs(title = TeX("$Baseline Control$ distribution for US Counties"), y = "count", x = "control (%)") +theme_bw() 
# 



ggplot(data_x, aes(control,y=..density.., fill = date)) + geom_histogram(bins =5 , position = "dodge", color = "white") + #geom_density(alpha = 0.2, bw = .04) + 
  scale_fill_manual(values = c("steelblue2", "lightgreen", "salmon")) + labs(title = TeX("$Baseline \\, Control$ distribution for US Counties"), y = "density", x = "control (%)") +theme_bw() + scale_x_continuous(breaks=c(0, 25, 50, 75, 100))
...
`r as.numeric (today()-ymd("2020-08-03"))` days ago: added county level "baseline control" and $R-e$ maps.    

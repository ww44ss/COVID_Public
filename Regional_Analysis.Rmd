...
p_total_cases <- ggplot(data = d_p, aes(x = date, y = total_cases) ) + 
  ...
  labs(title = "US Total Cases", subtitle = today(), y = "cases")+ 
    annotate(geom="text", x=(max(d_p$date) - days(50)), 
...
p_dailycases <- ggplot(d_p,
  ...
  geom_line(aes(y = daily_cases), color = "#2462A2", size = 1.1, alpha = 1) + 
    annotate(geom="text", x=(max(d_p$date) - days(40)), 
             ...
             label=str_c(round(max(d_p$daily_cases)/1000,2), " thousand daily cases    "), size = 3.0,
             color="steelblue4") +
  theme_bw()
...
data_x <- final_plot_data %>%
  filter(date == max(date) | date == ymd("2020-06-01")| date == ymd("2020-04-15")| date == ymd("2020-07-15")) %>%
...
ggplot(data_x, aes(r_fixed,y=..density.., fill = date)) + geom_histogram(bins =30 , position = "dodge") + geom_density(alpha = 0.2, bw = .04) + 
  scale_fill_manual(values = c("beige","steelblue2", "palegreen", "salmon")) + labs(title = "R_e distribution for all US Counties", y = "density", x = "R_e") +theme_bw() + scale_x_log10()

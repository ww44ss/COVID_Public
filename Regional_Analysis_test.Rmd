...
library(prettydoc)
library(latex2exp)
library(mgcv)  # test new spline model
library(bsts)
...
county_gps <- geoCounty %>%
  dplyr::dplyr::select(fips, lon, lat, rMapCounty) %>%
...
for(fips_x in fips_list) {
  ...
  if(nrow(fips_data) > 10) {
    ...
    ### messing around with prediction models
...
#   b_model<-NULL
#   pred1<-NULL
#   b_state<-AddLocalLinearTrend(list(), fips_data$log_cases)
#   b_model<-bsts(fips_data$log_cases, state.specification = b_state, niter = 100)
#   
#   pred1<-predict(b_model, horizon = 10)
# plot(pred1)
#   
#   model <- gam(log_cases ~ s(n), data = fips_data)
#   prediction_1 <- model %>% predict(1:nmax)
#   prediction_2 <- predict(spline_model_cases, x = 1:nmax)

  spline_model_cases <- smooth.spline(x = fips_data$n, y = fips_data$log_cases, spar = 0.65)
  spline_model_deaths <- smooth.spline(x = fips_data$n, y = fips_data$log_deaths, spar = 0.7)
  
  
  b <- bind_cols(x = prediction_2$x, old = prediction_2$y, new = as.numeric(prediction_1))
  b_1 <- b %>%
    mutate(old_case = 2^(old) - 1) %>%
    mutate(new_case = 2^(new) - 1) %>%
    yo
  
  c<-ggplot(b_1, aes(x = x) ) + geom_line(aes(y = old_case), color = "red") + geom_line(aes(y = new_case), color = "blue")
  ...
  temp_fips_data<-temp_fips_data[complete.cases(temp_fips_data),] %>% dplyr::select(fips, date, lat, lon, county, state, ST, cases, deaths, n, smoothed_cases, smoothed_deaths, daily_cases, daily_deaths, rolling_baseline_14, rolling_baseline_deaths, r_e, r_e_deaths)
...
## per capita cases
 ...
 state_abb <- bind_cols(state = state.name, ST = state.abb)
  ...
  data_max <- final_plot_data %>%
    ...
    dplyr::select(state, ST, county, per_capita, date) %>%
    yo
...
data_x <- final_plot_data %>%
  ...
  dplyr::select(ST, cases, daily_cases, rolling_baseline_14, r_e, population) %>%
  yo
...
data_x <- final_plot_data %>%
  ...
  #dplyr::select(ST, cases, daily_cases, rolling_baseline_14, r_e, population, persistence) %>%
  filter(date == max(date)) %>%
  ungroup() %>%  
  yo
...
plot_colors <- plot_colors[-c(1,(6:7))]
  ...
  data_max <- final_plot_data %>%
    ...
    filter(date == max_date) %>%
    dplyr::select(state, ST, county, control, date) %>%
...
plot_colors <- rev(plot_colors)
  ...
  data_max <- final_plot_data %>%
    ...
    #mutate(control = 100*(1-rolling_baseline_14/(max(rolling_baseline_14)+1))) %>%
    filter(date == max_date) %>%
    dplyr::select(state, ST, county, r_e, date) %>%
    mutate(county3 = county, 
           county = county %>% 
             str_replace_all(" ", "") %>% 
             str_replace_all("\\.", "") %>% 
...
data_x <- final_plot_data %>%
  ...
  summarize(cases = sum(cases), daily_cases = sum(daily_cases), rolling_baseline_14 = sum(rolling_baseline_14), population = sum(population))%>%
  mutate(r_e = 12*daily_cases/rolling_baseline_14) %>%
  dplyr::select(state, ST, cases, daily_cases, rolling_baseline_14, r_e, population) %>%
  yo
...
plot_x <- final_plot_data %>% 
  ...
  dplyr::select(date, cases, r_e, daily_cases, daily_actual) %>%
...
severe_county_table2 <- function(data_x = final_plot_data, state_x = "OR") {
  ...
  d_x <- data_x %>%
    ...
    dplyr::select(county, ST, 'case rank' = case_rank, 'severity' = severity_rank, "R_e" = R, cases, 'cases/100k' = per_capita, 'daily cases' = d_cases) %>%
    arrange(ST) %>%
    yo

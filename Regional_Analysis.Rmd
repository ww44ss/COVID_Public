...
for(fips_x in fips_list) {
  ...
  # spline_model_deaths <- smooth.spline(x = fips_data$n, y = fips_data$log_deaths, spar = .75, keep.data = F)
   ...
   df_x = as.integer(nmax/8 + 2)
  
  spline_model_cases <- smooth.spline(x = fips_data$n, y = fips_data$log_cases, keep.data = F, df=df_x)
  spline_model_deaths <- smooth.spline(x = fips_data$n, y = fips_data$log_deaths, keep.data = F, df=df_x)
...
`r as.numeric (today()-ymd("2020-08-20"))` days ago: adapter knot spacing for spline.   

...
for(fips_x in fips_list) {
  ...
  fips_data<-fips_data %>% 
    mutate(log_cases = log2(cases+.01)) %>% 
    mutate(log_deaths = log2(deaths+.01)) %>% 
    yo
...
## Deaths by State *since 2020-07-01* Per 100k Population    
...
ggplot(data = data_x, aes(x = votes_dem+votes_gop, y = 100000*(daily_deaths+.1)/population, color = winner, size = population))+
scale_color_manual(values = c("blue", "red"), name = "Candidate") +  
  geom_point(alpha = 0.3) + scale_y_log10() + scale_x_log10() +
  geom_smooth(method = "gam", formula = y ~ log10(x), linetype="dashed",
             color="darkred", fill="blue") + 
  labs(title = str_c("2016 Election Vote vs. Daily COVID deaths on ", data_x$date[1]), subtitle = "Voter and COVID data per US County", y = str_c("daily deaths per 100k population"), x = "% Vote for Trump") + 
  theme_bw() + 
...
ggplot(data = data_x, aes(x = population, y = 100000*(daily_deaths+.1)/population, color = 100*(1-hilary_fraction), size = population))+
scale_color_gradient(low = "blue", high = "red", name = "% Trump Vote") + 
  geom_point(alpha = 0.3) + scale_y_log10() + scale_x_log10() + 
  labs(title = str_c("2016 Election Vote vs. Daily COVID deaths on ", data_x$date[1]), subtitle = "Voter and COVID data per US County", y = str_c("daily deaths per 100k population"), x = "% Vote for Trump") + 
  theme_bw() + 

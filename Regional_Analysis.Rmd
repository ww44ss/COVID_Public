...
ggplot(data_x, aes(control,y=..density.., fill = date)) + geom_histogram(bins =4 , position = "dodge", color = "white") + #geom_density(alpha = 0.2, bw = .04) + 
  scale_fill_manual(values = c("steelblue2", "lightgreen", "salmon")) + labs(title = TeX("$Baseline \\, Control$ distribution for US Counties"), y = "density", x = "control (%)") +theme_bw() + scale_x_continuous(breaks=c(0, 33, 67, 100))

---
title: "POLITICAL COVID-19 STATISTICS"
author: "Saunders"
output:
  html_document:
    toc: true
...
d_p <- final_plot_data %>%
  ungroup()%>%
  group_by(date, party) %>%
  #(total_cases = sum(orig_cases), total_active = sum(rolling_baseline_14), total_deaths = sum(deaths), daily_cases = sum(daily_cases), total_daily = sum(daily_cases), daily_deaths = sum(daily_deaths), population = sum(population)) %>% 
  yo
...
p_total_cases <- ggplot(data = d_p, aes(x = date, y = total_cases, color = party) ) + 
  geom_line(size = 1.5) +
  labs(title = "US Total Cases", subtitle = "Governor Party Affiliation", y = "cases")+ 
...
p1_daily <- ggplot(data = d_p, aes(x = date, color = party) ) + 
 geom_line(aes(x = date, y = daily_cases), size = 0.8, alpha = 0.8)+
  labs(title = "Daily Cases", subtitle = "Governor Party Affiliation", y = "cases")+ 
  scale_color_manual(values = c("blue", "red"))+
...
p1_daily_deaths <- ggplot(data = d_p, aes(x = date, color = party) ) + 
 #geom_col(aes(y = daily_deaths), color = "grey80", width = 0.7, alpha = 0.5)+
  ...
  scale_color_manual(values = c("blue", "red"))+
  labs(title = "Daily Deaths", subtitle = "Governor Party Affiliation", y = "deaths")+ 
...
p_total_deaths <- ggplot(d_p, 
  ...
  scale_color_manual(values = c("blue", "red"))+
  labs(title = "Total Deaths", subtitle = "Governor Party Affiliation", y = "deaths")+
    # annotate(geom="text", x=(max(d_p$date) - days(40)), 
...
p_total_cases <- ggplot(data = d_p, aes(x = date, y = cases_per_100k, color = party) ) + 
  geom_line(size = 1.5) +
  labs(title = "Total Cases/100k", subtitle = "Governor Party Affiliation", y = "cases/100k")+ 
  scale_color_manual(values = c("blue", "red"))+
    # annotate(geom="text", x=(max(d_p$date) - days(60)), 
    #          y=max(0.95*d_p$total_cases), 
    #          label=str_c((max(d_p$total_cases)/1000000)%>% round(2), " million total cases    "), size = 3.0,
...
p1_daily_deaths <- ggplot(data = d_p, aes(x = date, color = party) ) + 
 #geom_col(aes(y = daily_deaths), color = "grey80", width = 0.7, alpha = 0.5)+
  ...
  scale_color_manual(values = c("blue", "red"))+
  labs(title = "Daily Deaths/100k", subtitle = "Governor Party Affiliation", y = "daily deaths/100k")+ 
  # annotate(geom="text", x=(max(d_p$date) - days(40)), 
  #            y=(0.05*max(d_p$daily_deaths)), 
  #            label=str_c("trend ", (d_p[d_p$date == max(d_p$date),]$daily_deaths[1])%>%round(0), " daily deaths    "), size = 3.0,
  #            color="steelblue4") +
...
p_total_deaths <- ggplot(d_p, 
  ...
  scale_color_manual(values = c("blue", "red"))+
  labs(title = "Total Deaths/100k", subtitle = "Governor Party Affiliation", y = "deaths")+
    # annotate(geom="text", x=(max(d_p$date) - days(40)), 
    #          y=max(0.05*d_p$total_deaths), 
    #          label=str_c((max(d_p$total_deaths)/1000) %>% round(1), " thousand deaths  "), size = 3.0,
    #          color="steelblue4") +
...
p_dailycases <- ggplot(d_p,
  ...
  labs(title = "Daily Cases/100k", subtitle = "Governor Party Affiliation", y = "cases/100k")+
  scale_color_manual(values = c("blue", "red"))+
  geom_line(size = 1.1, alpha = 1) + 
    # annotate(geom="text", x=(min(d_p$date) + days(60)), 
    #          y=max(0.95*d_p$daily_cases), 
...
state_list <- data_x %>% 
  ...
  select (state, 'R_e'=r_e , cases, daily_cases, per_capita, party )%>%
  arrange(desc(per_capita))%>%
  ungroup()%>%
  yo

ggplot(state_list, aes(x = fct_reorder(state, desc(per_capita)), y = per_capita, fill = party)) + geom_col() + scale_fill_manual(values = c("blue", "red")) + 
  labs(title = "COVID Cases per 100k by State", subtitle = "Governor's Party Affiliation", x = "State", y = "Cases per 100k") + 
  theme(axis.text.x = element_text(angle = 90, size = 5, vjust = 0.5, hjust = 1), legend.position = "top",legend.title = element_blank(),
  legend.text = element_text(color = "grey20", size = 8)) + theme_bw()

...
daily_deaths_trend <- d_p %>%
  ...
  rowwise()%>%
  summarize(daily_deaths = mean(c(fit, actual))) %>%
  as.numeric() %>%
  round(0)%>%
  yo
...
daily_per_capita <- ggplot(data = d_p, aes(x = date) ) + 
 geom_line(aes(x = date, y = 100000.*daily_cases2/us_pop), color = "#B91815", size = 0.8, alpha = 0.8)+
  labs(title = "Daily US Cases", subtitle = today(), y = "cases")+ 
   annotate(geom="text", x=(max(d_p$date) - days(30)), 
...
daily_deaths_per_capita <- ggplot(data = d_p, aes(x = date) ) + 
  geom_hline(yintercept =(100000*daily_deaths_trend/us_pop), color = "#A51541", size = 0.5 ) +
...
daily_cases_per_capita <- ggplot(data = d_p, aes(x = date) ) +
  geom_hline(yintercept =(100000*daily_cases_trend/us_pop), color = "#A51541", size = 0.5 ) +
 geom_col(aes(y = 100000.*daily_cases2/us_pop), color = "grey80", width = 0.7, alpha = 0.5)+
  ...
  annotate(geom="text", x=(min(d_p$date) + days(5)), 
             y=((100000*daily_cases_trend/us_pop)+1.1), 
...
grid.arrange(p_total_cases, p_daily_cases_per_capita, county_hist, p_re, p_mortality, p_daily_deaths, nrow = 3)

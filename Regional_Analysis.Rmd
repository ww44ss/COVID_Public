...
for(fips_x in fips_list) {
  ...
  spline_model_cases <- smooth.spline(x = fips_data$n, y = fips_data$log_cases, spar = 0.65)
  spline_model_deaths <- smooth.spline(x = fips_data$n, y = fips_data$log_deaths, spar = 0.65)
  
  
  ## use model to backward predict cases so windowed sum works
  
  nmax = max(fips_data$n)
  backward_prediction <- predict(spline_model_cases, x = -30:nmax)
  backward_prediction_1 <- tibble(n = backward_prediction$x, log_predicted_cases = backward_prediction$y)
  
  backward_prediction <- predict(spline_model_deaths, x = -30:nmax)
  ...
  backward_prediction2 <- backward_prediction %>%
    mutate(smoothed_cases = 2^log_predicted_cases) %>%
    mutate(smoothed_deaths = 2^(log_predicted_deaths)) %>%
    ...
    mutate(rolling_baseline_14= (roll_sum(daily_cases, 13, align = "right", fill = NA)) %>% round(0)) %>%
    mutate(rolling_baseline_14 = rolling_baseline_14-daily_cases) %>%
    
    
    mutate(rolling_baseline_deaths= (roll_sum(daily_deaths, 13, align = "right", fill = NA)) %>% round(0)) %>%
  ...
  temp_fips_data <- fips_data %>%
    ...
    filter(cases > 10) %>%
    mutate(r_fixed = (daily_cases/(rolling_baseline_14+5)*12)) %>%
    mutate(r_e_deaths = (daily_deaths/(rolling_baseline_deaths+1)*12)) %>%
    yo
  
  temp_fips_data<-temp_fips_data[complete.cases(temp_fips_data),] %>% select(fips, date, lat, lon, county, state, ST, cases, deaths, n, smoothed_cases, smoothed_deaths, daily_cases, daily_deaths, rolling_baseline_14, rolling_baseline_deaths, r_fixed, r_e_deaths)
...
file_name_date_append <- today() %>%
  ...
  #dir<-"C:/Users/wisaund/OneDrive - Microsoft/Documents/Github/NYTimesCovid"

  saved_modified_files <- list.files("~/")
  ...
  if(already_created){
    ...
    final_plot_data <- read_csv(str_c("~/", file_name_date))
...
- [here]([https://rpubs.com/ww44ss/NATIONAL_TOTAL_COVID_CASES).   
...
```{r, warning = F, message = F, fig.align='center', fig.height =14, fig.width = 12, echo = F, eval = F}

## plot per capita cases
## This is deprecated (eval = F)
 ...
 state_abb <- bind_cols(state = state.name, ST = state.abb)
  ...
  plot_colors <- c("#0020FF33", "darkred")
  ...
  q1 <- p + geom_point(data = data_x %>% 
                        filter(date == max(date)|date == max(date) - days(28))%>%   
                        arrange(desc(cases))%>%filter(cases>10), 
                      aes(x = lon, y = lat, fill = per_capita, 
                          group = fips), size = 3, pch=21, color = "#FFFFFF00") +
    #geom_sf_text(labels, mapping = aes(label = state_abbv), color = "black", size = 4)+
    theme(legend.position="top") +
    xlim(bbox[1], bbox[3]) +
    ylim(bbox[2], bbox[4])+
    #scale_size_continuous( breaks=c(1, 8, 64, 512, 4096), range = c(1, 15))
    theme_bw()+
    ...
    labs(title = str_c('ONE MONTH GROWTH OF PER CAPITA COVID-19 CASES : ', date_display), 
         subtitle=str_c("NYTimes COVID19 Database"), "color = cases/100k") +
...
```{r, warning = F, message = F, fig.align='center', fig.height =11, fig.width = 9, echo = F, eval = T}
 ...
 state_abb <- bind_cols(state = state.name, ST = state.abb)
  ...
  counties <- counties_max %>%
    ...
    theme(axis.title.x=element_blank(),
          ...
          panel.grid.minor = element_blank(),
          strip.text = element_text(color = "#154555", size = 12, face = "bold")) +
    labs(title = str_c('PER CAPITA COVID-19 CASES : ', date_display),
         subtitle=str_c("NYTimes COVID19 Database"), "color = cases/100k") +
     annotate(geom="text", x=-70,
             y=25,
             label=str_c("ww44ss - ", today() ), size = 2.5,
...
## per capita deaths
 ...
 state_abb <- bind_cols(state = state.name, ST = state.abb)
  ...
  #plot_colors <- rev(c(brewer.pal(n = 5, name = "RdYlGn"), "#3392B2"))
  plot_colors <- c("white", "darkslateblue")
  ...
  counties <- counties_max %>%
    ...
    theme(axis.title.x=element_blank(),
          ...
          panel.grid.minor = element_blank(),
          strip.text = element_text(color = "#154555", size = 12, face = "bold")) +
    labs(title = str_c('PER CAPITA COVID-19 MORTALITY : ', date_display),
         subtitle=str_c("NYTimes COVID19 Database"), "color = cases/100k") +
     annotate(geom="text", x=-70,
             y=25,
             label=str_c("ww44ss - ", today() ), size = 2.5,
...
```{r, warning = F, message = F, fig.align='center', fig.height =12, fig.width = 12, echo = F, eval = T}
 ...
 state_abb <- bind_cols(state = state.name, ST = state.abb)
  ...
  plot_colors <- c(brewer.pal(n = 8, name = "YlGnBu"))
  ...
  data_x <- data_x %>%
    ...
    filter(rolling_baseline_14 == max(rolling_baseline_14)) %>%
    arrange(desc(date)) %>%
    mutate(count = row_number()) %>%
    filter(count == 1) %>%
    select(-count) %>%
    ungroup()%>%
    mutate(control = ifelse(date <  (today() - days(7)),"CONTAINED" ,"UNCONTAINED")) %>%
    yo
    
  p <- 
    ggplot() + 
    geom_sf(data = states , fill = "#E5D9CC88", color = "white", size = 1.0) + 
    coord_sf(xlim = c(as.numeric(bbox[1]), as.numeric(bbox[3])), ylim = c(as.numeric(bbox[2]), as.numeric(bbox[4])), expand = FALSE) + 
    theme_bw()

  
  q1 <- p + geom_point(data = data_x, 
                      aes(x = lon, y = lat, size = rolling_baseline_14, 
                          group = fips, fill = control), pch=21, alpha = 0.5, color = "grey20") + 
    theme(legend.position="top") +
    xlim(bbox[1], bbox[3]) +
    ylim(bbox[2], bbox[4])+
    scale_size_continuous( breaks=c(8, 64, 512, 4096, 32768), range = c(1, 40), name = "cases at peak")+ theme_bw()+
    theme(axis.title.x=element_blank(),
          axis.text.x=element_blank(),
          axis.ticks.x=element_blank(),
          axis.title.y=element_blank(),
    ...
    labs(title = str_c("CONTAINED AND UNCONTAINED DISEASE ", today()),
         subtitle = "circle indicates active cases at peak") +
     annotate(geom="text", x=-70, 
             y=25, 
             label=str_c("ww44ss - ", today() ), size = 2.5,
             color="Grey30")+
...
```{r, warning = F, message = F, error = F, fig.align='center', fig.height =4, fig.width = 8, echo = F, eval = T}

## plot reproduction rate

states <- states_cache %>% filter(!is.na(ST))
...
states <- states %>% 
  ...
  range_lon = abs(bbox[1] - bbox[3])
    ...
    ggplot() + 
    geom_sf(data = states, aes(geometry = geom, fill = log2(r_e)), color = "white", size = 0.6, alpha = 0.85) + 
    coord_sf(xlim = c(as.numeric(bbox[1]), as.numeric(bbox[3])), ylim = c(as.numeric(bbox[2]), as.numeric(bbox[4])), expand = FALSE) +
    scale_fill_gradientn(colors =plot_colors, name = "log2(R)", limits = c(-0.61, 0.61)) + 
    theme_bw()+
    theme(axis.title.x=element_blank(),
          axis.text.x=element_blank(),
          axis.ticks.x=element_blank(),
          ...
          panel.grid.minor = element_blank(),
          strip.text = element_text(color = "#154555", size = 14, face = "bold")) +
    labs(title = str_c("Reproduction Rate, R_e: ", today()),
         subtitle = "Disease reduction required log2(R_e) < 0") +
     annotate(geom="text", x=-70, 
             y=25, 
             label=str_c("ww44ss - ", today() ), size = 2.5,
             color="Grey30")
...
data_x <- final_plot_data %>%
  ...
  summarize(cases = sum(cases), daily_cases = sum(daily_cases), rolling_baseline_14 = sum(rolling_baseline_14), population = sum(population))%>%
  mutate(r_e = 12*daily_cases/rolling_baseline_14) %>%
  mutate(max_rolling_baseline_14 = max(rolling_baseline_14)) %>%
  mutate(persistence = 100-100*rolling_baseline_14/max_rolling_baseline_14) %>%
  #select(ST, cases, daily_cases, rolling_baseline_14, r_e, population, persistence) %>%
  filter(date == max(date)) %>%
  ungroup() %>%  
  yo
...
states <- states %>% 
  ...
  range_lon = abs(bbox[1] - bbox[3])
    ...
    ggplot() + 
    geom_sf(data = states, aes(geometry = geom, fill = persistence), color = "white", size = 0.6, alpha = 0.85) + 
    coord_sf(xlim = c(as.numeric(bbox[1]), as.numeric(bbox[3])), ylim = c(as.numeric(bbox[2]), as.numeric(bbox[4])), expand = FALSE) +
    ...
    theme(axis.title.x=element_blank(),
          ...
          panel.grid.minor = element_blank(),
          strip.text = element_text(color = "#154555", size = 14, face = "bold")) +
    labs(title = str_c("Disease Control: ", today()),
         subtitle = "Low control indicates high relative presence of active disease risk") +
    annotate(geom="text", x=-70, 
             y=25, 
             label=str_c("ww44ss - ", today() ), size = 2.5,
             color="Grey30")
...
d_p <- final_plot_data %>%
  ...
  mutate(r_fixed = daily_cases*12/total_active) %>%
...
p_total_cases <- ggplot(data = d_p, aes(x = date, y = total_cases) ) + 
 ...
 #geom_line(aes(x = date, y = total_active), color = "salmon", size = 1.5, alpha = 0.8)+
  labs(title = "US Total Cases", subtitle = today(), y = "cases")+ 
    annotate(geom="text", x=(max(d_p$date) - days(50)), #xend=(max(d_p$date) - days(25)),
             y=max(0.95*d_p$total_cases), 
             label=str_c(max(d_p$total_cases), " total cases    "), size = 2.5,
             color="steelblue4") +
  # annotate(geom="text", x=(max(d_p$date) - days(13)), #xend=(max(d_p$date) - days(25)),
  #            y=(1.4*d_p[d_p$date == max(d_p$date),]$total_active[1]), 
  #            label=str_c(d_p[d_p$date == max(d_p$date),]$total_active[1], " active cases"), size = 2.5,
  #            color="steelblue") +
  theme_bw()
...
p1_daily <- ggplot(data = d_p, aes(x = date) ) + 
  #geom_line(size = 0.8, color = "grey40") +
 geom_line(aes(x = date, y = daily_cases2), color = "#891815", size = 1.0, alpha = 0.8)+
  ...
  annotate(geom="text", x=(max(d_p$date) - days(30)), 
             ...
             label=str_c((d_p[d_p$date == max(d_p$date),]$daily_cases2[1])%>%round(0), " daily cases"), size = 2.5,
             color="steelblue4") +
  theme_bw()

p1_daily_deaths <- ggplot(data = d_p, aes(x = date) ) + 
 geom_line(aes(y = daily_deaths2), color = "#891815", size = 1.0, alpha = 0.8)+
  ...
  annotate(geom="text", x=(max(d_p$date) - days(40)), 
             ...
             label=str_c("trend ", (d_p[d_p$date == max(d_p$date),]$daily_deaths[1])%>%round(0), "deaths daily"), size = 2.5,
             color="steelblue4") +
  theme_bw()
...
p_mortality <- ggplot(d_p, 
  ...
  annotate(geom="text", x=(max(d_p$date) - days(40)), 
             ...
             label=str_c((d_p[d_p$date == max(d_p$date),]$mortality[1])%>%round(2), "% mortality on ", max(d_p$date)), size = 3.5,
             color="steelblue4") +
  theme_bw()
...
p_daily_mortality <- ggplot(d_p, 
  ...
  annotate(geom="text", x=(max(d_p$date) - days(40)), 
             ...
             label=str_c((d_p[d_p$date == max(d_p$date),]$daily_mortality[1])%>%round(2), "% mortality"), size = 2.5,
             color="steelblue4") +
  theme_bw()
...
p4 <- ggplot(d_p, 
  ...
  labs(title = "Total US Deaths", subtitle = today(), y = "deaths")+
    annotate(geom="text", x=(max(d_p$date) - days(40)), 
             ...
             label=str_c(max(d_p$total_deaths), " deaths on ", max(d_p$date) ), size = 2.5,
             color="steelblue4") +
  theme_bw()


p_dailycases <- ggplot(d_p,
                       aes(x = date, y = daily_cases)) + geom_line(color = "#2462A2", size = 1, alpha = 0.8) + 
  labs(title = "US Daily Cases", subtitle = today(), y = "cases")+
    annotate(geom="text", x=(max(d_p$date) - days(40)), 
             y=max(0.05*d_p$daily_cases), 
             label=str_c(round(max(d_p$daily_cases),0), " max daily cases on ", max(d_p$date) ), size = 2.5,
             color="steelblue4") +
  theme_bw()


p_r <- ggplot(d_p,
                       aes(x = date, y = r_fixed)) + geom_line(color = "#5442E2", size = 1.5, alpha = 0.8) + 
  labs(title = "Effective Reproduction Rate", subtitle = today(), y = "R_e")+
  scale_y_log10()+
  annotate(geom="text", x=(max(d_p$date) - days(40)), 
             y=0.85*max(d_p$r_fixed), 
             label=str_c("R_e = ",(d_p[d_p$date == max(d_p$date),]$r_fixed[1])%>%round(2), " on ", max(d_p$date)), size = 3.5,
             color="steelblue4") +
  theme_bw()



grid.arrange(p_total_cases, p_dailycases, p4, p1_daily_deaths,  nrow = 2)
...
grid.arrange(p_mortality,nrow = 1)
...
```{r, fig.height=4, fig.width=7, fig.align="center"}
data_x <- final_plot_data %>%
  filter(date == max(date) | date == ymd("2020-06-01")| date == ymd("2020-04-15")) %>%
  filter(is.finite(r_fixed)) %>%
  mutate(date = as.factor(date))%>%
  filter(r_fixed < 3 & r_fixed>0)%>%
  select(r_fixed, date) %>%
  yo


ggplot(data_x, aes(r_fixed,y=..density.., fill = date)) + geom_histogram(bins =30 , position = "dodge") + geom_density(alpha = 0.2, bw = .14) + 
  scale_fill_manual(values = c("steelblue2", "palegreen", "salmon")) + labs(title = "R_e distribution for all US Counties", y = "density", x = "R_e") +theme_bw() + scale_x_log10()
...
Region_Plot <- function(data_x = final_plot_data, state_y = c("WA", "OR", "CA"))

{
  ...
  data_x <- data_x %>% 
    mutate(r_fixed = ifelse(r_fixed > 5, 5, r_fixed))
 ...
 size_max <- ifelse("NY" %in% state_y, 100000, 30000)
  ...
  q <- p + geom_point(data = data_x %>% 
                        ...
                        arrange(desc(cases))%>%
                        filter(cases>10), 
                      aes(x = lon, y = lat,  size = daily_cases, fill = log2(r_fixed), 
...

{
 ...
 size_max <- ifelse("NY" %in% state_y, 100000, 30000)
  ...
  q <- p + geom_point(data = data_x %>% 
                        ...
                        arrange(desc(cases))%>%
                        filter(cases>10), 
                      aes(x = lon, y = lat,  size = daily_cases, fill = r_fixed, 
                          group = fips), pch=21, alpha = 1.0, color = "grey20") +
    theme(legend.position="top") +
    xlim(bbox[1], bbox[3]) +
    ylim(bbox[2], bbox[4])+
...
plot_x <- final_plot_data %>% 
  ...
  select(date, cases, r_fixed, daily_cases, daily_actual) %>%
...
daily_case_plot <- ggplot(plot_x, 
       aes(x = date, y =daily_cases, color = log2(r_fixed+.1) )) + 
...
total_case_plot <- ggplot(final_plot_data %>% filter(county == county_x & ST == state_x), 
       aes(y = cases, x = date) ) + 
    geom_point(aes(y = orig_cases), fill = "grey10", color = "grey80", alpha = 0.5, size = 2.0, pch = 21)+
  geom_line(size = 2.5, color = "steelblue3", alpha = 0.4) +
    ylim(0, NA) +
    xlim(ymd("2020-03-01"), NA)+
  theme_bw() + 
  theme(legend.position="none") +
...
Rep_plot <- ggplot(final_plot_data %>% filter(county == county_x & ST == state_x), 
       aes(x = date, y =r_fixed, color = log2(r_fixed) )) + 
    geom_line(size = 2.0) +
  scale_color_gradientn(colors = plot_colors, name = expression(paste("log2(", italic(R),")")), limits = c(-2.2, 2.2)) + 
  #scale_y_log10() +
  theme_bw() + 
...
d_p <- data_x %>%
  ...
  ungroup()%>%
  group_by(date, state) %>%
  summarize(total_cases = sum(orig_cases), total_daily = sum(daily_cases), total_active = sum(rolling_baseline_14), total_deaths = sum(deaths), population = sum(population)) %>%
...
d_c <- data_x %>%
  ...
  summarize(total_cases = sum(orig_cases), total_daily = sum(daily_cases), total_active = sum(rolling_baseline_14), total_deaths = sum(deaths)) %>%
  mutate(r_e = 12*total_daily/total_active) %>%
...
p_daily_cases <-  ggplot(data = d_p, aes(x = date, color = state))  + 
 geom_line(aes(x = date, y = total_daily, color = state), linetype = 1, size = 1, alpha = 0.8)+
...
grid.arrange(p_total_cases, p_daily_cases, county_hist, p_re, p_mortality, p_daily_cases_per_capita, nrow = 3)
...
severe_county_table2 <- function(data_x = final_plot_data, state_x = "OR") {
  ...
  d_x <- data_x %>%
    ...
    mutate(projected_new_cases = (1/12)*rolling_baseline_14*(2.7^(r_fixed*6))) %>%
    ...
    #arrange(case_rank) %>%
    mutate(cases = cases%>%round(0), projected_new_cases = projected_new_cases %>% round(0), R = round(r_fixed, 1), per_capita = 10*round(10000*cases/population,0), daily_per_capita = round(100000*daily_cases/population,0), d_cases = round(daily_cases, 0)) %>%
...
```{r, warning = F, message = F, fig.align='center', fig.width = 9, fig.height = 8, echo = F, results = 'asis'}
...
```{r}
county_plot(final_plot_data, county_x = "Collin", state_x = "TX")
...
```{r}
county_plot(final_plot_data, county_x = "Travis", state_x = "TX")

```

```{r}
county_plot(final_plot_data, county_x = "Harris", state_x = "TX")
...
```{r}
county_plot(final_plot_data, county_x = "Hartford", state_x = "CT")
...
```{r}
county_plot(final_plot_data, county_x = "Providence", state_x = "RI")
...
```{r}
county_plot(final_plot_data, county_x = "Mecklenburg", state_x = "NC")
...
```{r}
county_plot(final_plot_data, county_x = "Richland", state_x = "SC")
...
```{r}
county_plot(final_plot_data, county_x = "Gallatin", state_x = "MT")

```

```{r}
county_plot(final_plot_data, county_x = "Ada", state_x = "ID")
...
```{r}
county_plot(final_plot_data, county_x = "Kane", state_x = "IL")
...
## version history    
Today is `r today()`.    
`r as.numeric(today()-ymd("2020-05-20"))` days ago: Multiple states.   
`r as.numeric(today()-ymd("2020-05-28"))` days ago: $R_e$ computation.     
...
`r as.numeric (today()-ymd("2020-06-27"))` days ago: Deprecated national map.   
...
`r as.numeric (today()-ymd("2020-07-20"))` days ago: changed to county boundarieson national map for per capita disease. 

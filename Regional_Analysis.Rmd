...
for(fips_x in fips_list) {
  ...
  spline_model_cases <- smooth.spline(x = fips_data$n, y = fips_data$log_cases, spar = 0.5)
  spline_model_deaths <- smooth.spline(x = fips_data$n, y = fips_data$log_deaths, spar = 0.5)
  
  ## experiment with reducing spar from 0.65 to 0.5
...
```{r, warning = F, message = F, fig.align='center', fig.height =6, fig.width = 9, echo = F, eval = T}
...
plot_colors <- plot_colors[-c(1,(6:7))]
  ...
  counties_max <- counties %>%
    ...
    ggplot(crs = st_crs(102003)) + 
    geom_sf(data = counties_max, aes(geometry = geom, fill = per_capita), color = "#DDDDCC77", size = 0.2 ) + 
    coord_sf(xlim = c(as.numeric(bbox[1]), as.numeric(bbox[3])), ylim = c(as.numeric(bbox[2]), as.numeric(bbox[4])), expand = FALSE) + 
    scale_fill_gradientn(colors =plot_colors, name = "Control (%)", limits = c(0, 100)) + 
    theme_bw() +
    theme(axis.title.x=element_blank(),
          ...
          panel.grid.minor = element_blank(),
          strip.text = element_text(color = "#154555", size = 12, face = "bold")) +
    labs(title = str_c('COUNTY COVID-19 CONTROL : ', date_display),
         subtitle=str_c("NYTimes COVID19 Database"), "color = cases/100k") +
     annotate(geom="text", x=-70,
             y=25,
             label=str_c("ww44ss - ", today() ), size = 2.5,
...
total_case_plot <- ggplot(final_plot_data %>% filter(county == county_x & ST == state_x), 
       aes(y = cases, x = date) ) + 
    geom_line(size = 2.0, color = "beige", alpha = 0.5) +
...
p_daily_cases <-  ggplot(data = d_p, aes(x = date))  + 
  ...
  geom_point(aes(x = date, y = orig_daily), color = "gray80",size = 1.5, alpha = 0.2)+
...
p_daily_deaths <- ggplot(d_p, 
  ...
  geom_point(aes(y = daily_deaths), color = "grey80", size = 1.5)+

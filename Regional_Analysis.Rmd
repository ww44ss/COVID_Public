...
data_x <- final_plot_data %>%
  ...
  mutate(short_term_change = daily_cases*(0.5*r_e^2))%>%
  yo
...
t_cases <-    
  ggplot() + 
    ...
    scale_fill_gradientn(colors =plot_colors, name = TeX("Cases"), limits = c(0, 800000)) + 
    theme_bw()+
...
p_per_capita <-    
  ggplot() + 
    ...
    coord_sf(xlim = c(as.numeric(bbox[1]), as.numeric(bbox[3])), ylim = c(as.numeric(bbox[2]), as.numeric(bbox[4])), expand = FALSE) +
    scale_fill_gradientn(colors =plot_colors, name = TeX("Cases/100k"), limits = c(0, 50)) + 
    theme_bw()+
    theme(axis.title.x=element_blank(),
          axis.text.x=element_blank(),
          axis.ticks.x=element_blank(),

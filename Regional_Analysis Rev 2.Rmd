...
```{r, warning=F, error=F, fig.align='center', message = F, error=F, warning = F, fig.height=3.5, fig.width = 6, eval = F}


###

data_x <- final_plot_data %>%
  filter(date == max(date) | date == (max(date) - days(7))) %>%
  yo
...
for (state_x in state_list) {
  ...
  d_p_2 <- d_p %>%
    filter(date == max(date)) %>%
    mutate(date = date + days(7), cases = cases * (1+(1-r_e)/5)) 
 ...
 pp<- ggplot(d_p,
    ...
    labs(title = str_c("Cases per day per 100k population for counties in ", state_x), x = "county", y = "cases per day per 100k", fill = "date") + scale_fill_manual(values = c("#2244DD99", "#1111DD", "salmon"))+
...
grid.arrange(p_total_cases, p_daily_cases_per_capita, county_hist, p_re, p_mortality, p_daily_deaths_per_capita, nrow = 3)

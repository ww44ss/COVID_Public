...
p_total_cases <- ggplot(data = d_p, aes(x = date, y = total_cases, color = party) ) + 
  geom_line(size = 1.5) +
  labs(title = "US Total Cases by State", subtitle = "State Governor's party affiliation indicated", y = "cases")+ 
...
p1_daily <- ggplot(data = d_p, aes(x = date, color = party) ) + 
 geom_line(aes(x = date, y = daily_cases), size = 0.8, alpha = 0.8)+
  labs(title = "Daily Cases by State", subtitle = "State Governor's party affiliation indicated", y = "cases")+ 
  scale_color_manual(values = c("blue", "red"))+
...
p1_daily_deaths <- ggplot(data = d_p, aes(x = date, color = party) ) + 
 #geom_col(aes(y = daily_deaths), color = "grey80", width = 0.7, alpha = 0.5)+
  ...
  scale_color_manual(values = c("blue", "red"))+
  labs(title = "Daily Deaths by State", subtitle = "By State Governor's party affiliation indicated", y = "deaths")+ 
...
p_total_deaths <- ggplot(d_p, 
  ...
  scale_color_manual(values = c("blue", "red"))+
  labs(title = "Total Deaths by State", subtitle = "State Governor's party affiliation indicated", y = "deaths")+
    # annotate(geom="text", x=(max(d_p$date) - days(40)), 
...
p_total_cases <- ggplot(data = d_p, aes(x = date, y = cases_per_100k, color = party) ) + 
  geom_line(size = 1.5) +
  labs(title = "Total Cases, by State, per 100k Population", subtitle = element_text("State Governor's party affiliation indicated", size = 3), y = "cases/100k")+ 
  scale_color_manual(values = c("blue", "red"))+
    # annotate(geom="text", x=(max(d_p$date) - days(60)), 
    #          y=max(0.95*d_p$total_cases), 
    #          label=str_c((max(d_p$total_cases)/1000000)%>% round(2), " million total cases    "), size = 3.0,
...
p1_daily_deaths <- ggplot(data = d_p, aes(x = date, color = party) ) + 
 #geom_col(aes(y = daily_deaths), color = "grey80", width = 0.7, alpha = 0.5)+
  ...
  scale_color_manual(values = c("blue", "red"))+
  labs(title = "Daily Deaths, by State, per 100k Population", subtitle = element_text("State Governor's party affiliation", size = 3), y = "daily deaths/100k")+ 
  # annotate(geom="text", x=(max(d_p$date) - days(40)), 
  #            y=(0.05*max(d_p$daily_deaths)), 
  #            label=str_c("trend ", (d_p[d_p$date == max(d_p$date),]$daily_deaths[1])%>%round(0), " daily deaths    "), size = 3.0,
  #            color="steelblue4") +
...
p_total_deaths <- ggplot(d_p, 
  ...
  scale_color_manual(values = c("blue", "red"))+
  labs(title = "Total Deaths, by State, per 100k Population", subtitle = element_text("State Governor's party affiliation", size = 3), y = "deaths")+
    # annotate(geom="text", x=(max(d_p$date) - days(40)), 
    #          y=max(0.05*d_p$total_deaths), 
    #          label=str_c((max(d_p$total_deaths)/1000) %>% round(1), " thousand deaths  "), size = 3.0,
    #          color="steelblue4") +
...
p_dailycases <- ggplot(d_p,
  ...
  labs(title = "Daily Cases, by State, per 100k Population", subtitle = element_text("State Governor's party affiliation indicated", size = 3), y = "cases/100k")+
  scale_color_manual(values = c("blue", "red"))+
  geom_line(size = 1.1, alpha = 1) + 
    # annotate(geom="text", x=(min(d_p$date) + days(60)), 
    #          y=max(0.95*d_p$daily_cases), 
...
Many State Governors relaxed as early as Mid May the stay at home orders implemented to protect the public from COVID-19. Subequently cases surged, resulting in what were largely preventable deaths.   

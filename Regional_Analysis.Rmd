...
daily_per_capita <- ggplot(data = d_p, aes(x = date) ) + 
 geom_line(aes(x = date, y = 100000.*daily_cases2/us_pop), color = "#891815", size = 0.8, alpha = 0.8)+
  labs(title = "Daily US Cases", subtitle = today(), y = "cases")+ 
   annotate(geom="text", x=(max(d_p$date) - days(30)), 
...
daily_deaths_per_capita <- ggplot(data = d_p, aes(x = date) ) + 
  geom_hline(yintercept =(100000*daily_deaths_trend/us_pop), color = "151591", size = 0.5 ) +
 geom_col(aes(y = 100000.*daily_deaths2/us_pop), color = "grey80", width = 0.7, alpha = 0.5)+
  geom_line(aes(y = 100000*daily_deaths/us_pop), size = 1.1, color = "#151589AA")+
  labs(title = "Daily US Deaths per 100k Population", subtitle = str_c("Current Estimated US trend is ", daily_deaths_trend, " deaths per day"), y = "deaths per 100k")+ 
  annotate(geom="text", x=(min(d_p$date) + days(5)), 
             y=((100000*daily_deaths_trend/us_pop)+.03), 
...
daily_cases_per_capita <- ggplot(data = d_p, aes(x = date) ) +
  geom_hline(yintercept =(100000*daily_cases_trend/us_pop), color = "151591", size = 0.5 ) +
 geom_col(aes(y = 100000.*daily_cases2/us_pop), color = "grey80", width = 0.7, alpha = 0.5)+
  geom_line(aes(y = 100000*daily_cases/us_pop), size = 1.1, color = "#891515AA")+
  labs(title = "Daily US Cases per 100k Population", subtitle = str_c("Current Estimated US trend is ", daily_cases_trend, " cases per day"), y = "cases per 100k")+ 
  annotate(geom="text", x=(min(d_p$date) + days(5)), 
             y=((100000*daily_cases_trend/us_pop)+1), 
             label=str_c((100000*daily_cases_trend/us_pop)%>%round(2),  " deaths/100k"), size = 3.0,
             color="steelblue4", hjust = 0) +
  annotate(geom="text", x=(max(d_p$date) - days(10)), 

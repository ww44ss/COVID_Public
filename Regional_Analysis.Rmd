...
```{r, warning = F, message = F, fig.align='center', fig.height =7, fig.width = 9, echo = F, eval = T}
  ...
  q1 <- p + geom_point(data = data_x, 
    ...
    labs(title = str_c("CONTAINED AND UNCONTAINED DISEASE ", today()),
     ...
     annotate(geom="text", x=-70, 
             ...
             label=str_c("ww44ss - ", today() ), size = 2.5,
             color="Grey30")+
    scale_fill_manual(values = c("steelblue", "salmon")) #+facet_grid(control~.)
 ...
 #geom_line(aes(x = date, y = total_active), color = "salmon", size = 1.5, alpha = 0.8)+
  labs(title = "US Total Cases", subtitle = today(), y = "cases")+ 
    annotate(geom="text", x=(max(d_p$date) - days(50)), #xend=(max(d_p$date) - days(25)),
             ...
             label=str_c((max(d_p$total_cases)/100000)%>% round(2), " million cases    "), size = 3.0,
             color="steelblue4") +
...
p1_daily_deaths <- ggplot(data = d_p, aes(x = date) ) + 
 geom_line(aes(y = daily_deaths2), color = "#891815", size = 1.0, alpha = 0.8)+
  ...
  annotate(geom="text", x=(max(d_p$date) - days(40)), 
             ...
             label=str_c("trend ", (d_p[d_p$date == max(d_p$date),]$daily_deaths[1])%>%round(0), " deaths    "), size = 3.0,
             color="steelblue4") +
  theme_bw()
...
p_dailycases <- ggplot(d_p,
  ...
  labs(title = "US Daily Cases", subtitle = today(), y = "cases")+
    annotate(geom="text", x=(max(d_p$date) - days(40)), 
             ...
             label=str_c(round(max(d_p$daily_cases)/1000,2), " thousand cases    "), size = 3.0,
             color="steelblue4") +
  theme_bw()
...
p_r <- ggplot(d_p,
  ...
  annotate(geom="text", x=(max(d_p$date) - days(40)), 
             ...
             label=str_c("R_e = ",(d_p[d_p$date == max(d_p$date),]$r_fixed[1])%>%round(2)), size = 3.0,
             color="steelblue4") +
  theme_bw()

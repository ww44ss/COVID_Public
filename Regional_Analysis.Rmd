...
daily_deaths_per_capita <- ggplot(data = d_p, aes(x = date) ) + 
 ...
 geom_col(aes(y = 100000.*daily_deaths2/us_pop), color = "grey80", width = 0.7, alpha = 0.5)+
  ...
  labs(title = "Daily US Deaths per 100k Population", subtitle = str_c("US trend is ", daily_deaths_trend, " deaths per day"), y = "deaths per 100k")+ 
  annotate(geom="text", x=(min(d_p$date) + days(5)), 
             y=((100000*daily_deaths_trend/us_pop)+.04), 
             label=str_c((100000*daily_deaths_trend/us_pop)%>%round(3),  " deaths/100k"), size = 3.0,
...
daily_cases_per_capita <- ggplot(data = d_p, aes(x = date) ) +
 ...
 geom_col(aes(y = 100000.*daily_cases2/us_pop), color = "grey80", width = 0.7, alpha = 0.5)+
  ...
  labs(title = "Daily US Cases per 100k Population", subtitle = str_c("US trend is ", daily_cases_trend, " cases per day"), y = "cases per 100k")+ 
  annotate(geom="text", x=(min(d_p$date) + days(5)), 
             y=((100000*daily_cases_trend/us_pop)*1.05), 
             label=str_c((100000*daily_cases_trend/us_pop)%>%round(2),  " cases/100k"), size = 3.0,
             color="steelblue4", hjust = 0) +
  annotate(geom="text", x=(max(d_p$date) - days(10)), 
...
p_daily_mortality <- ggplot(d_p%>%filter(date > ymd("2020-03-15")), 
  ...
  annotate(geom="text", x=(max(d_p$date) - days(40)), 
             y=0.85*max(d_p$daily_mortality), 
...
plot_x <- final_plot_data %>% 
  ...
  #mutate(daily_actual = ifelse(daily_actual > 1.1*max(daily_cases), (1+runif(n=1)*0.1)*max(daily_cases), daily_actual)) %>% # this dummifies the max for plotting.
  filter(is.finite(daily_actual)) %>%
  yo

daily_case_plot <- ggplot(plot_x, 
       aes(x = date, y =100000*daily_cases/population + 1, color = log2(r_e+.1) )) + 
  geom_col(aes(y = 100000*daily_actual/population+1), fill = "grey60", color = "transparent",alpha = 0.3, size = 0.6) +
    geom_line(size = 2.0) +
  ...
  labs(subtitle = expression(paste("Daily Cases per 100k population")), y = "cases", x = "date")
